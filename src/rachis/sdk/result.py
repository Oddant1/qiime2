...
class Artifact(Result):
    ...
    def get_provenance_path(self):
        return os.path.join(self._archiver.root_dir, 'provenance')
    ...
    def redact_metadata(self):
        ...
        # Filter out lines refering to the now missing metadata files
        for file in Path(prov_path).parent.iterdir():
            if 'checksums' in file.name:
                with open(file, 'r') as f:
                    lines = f.readlines()

                lines = [
                    line for line in lines if not any(
                        r_path in line for r_path in relative_metadata_paths
                    )
                ]
