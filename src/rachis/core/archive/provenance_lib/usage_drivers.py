...
def build_header(
    ...
    '''
    rachis_md = metadata('rachis')
    vzn = rachis_md['Version']
    ts = datetime.now()
    header = []
    ...
    header.extend([
        f'# Auto-generated by Rachis v.{vzn} at '
    ...
    header.append(
        '# For User Support, post to the QIIME 2 Forum at '
...
def build_footer(dag: ProvDAG, boundary: str) -> List[str]:
    ...
    footer.append(
        '# The following Rachis Results were parsed to produce this script:'
    )
    footer.extend(pairs)
    footer.append(boundary)
    footer.append('')
...
class ReplayPythonUsage(ArtifactAPIUsage):
    ...
    copyright = importlib.resources.read_text(
        'rachis.core.archive.provenance_lib.assets',
        'copyright_note.txt'
        ).split('\n')
    ...
    def _plugin_import_as_name_not_found(self, action):
        base = f'rachis.plugins.{action.plugin_id}.actions'
    ...
    class repr_raw_variable_name:
        ...
        # (as in rachis.plugins.ArtifactAPIUsageVariable)
